...
// Only well-known, non-adult people with an actual acting/directing/writing credit history
const GOOD_DEPT = new Set(['Acting', 'Directing', 'Writing', 'Production']);
...
export const GET = async ({ params, locals }) => {
  ...
  const mv = name.match(/^(movies|tv)-(\d+)$/);
  if (mv) {
    const type = mv[1];
  ...
  // People files: people-1, people-2 ...
  const pm = name.match(/^people-(\d+)$/);
  if (pm) {
    const n = parseInt(pm[1], 10);
    if (n < 1 || n > PEOPLE_SITEMAPS) return new Response('Not found', { status: 404 });

    const start = (n - 1) * PEOPLE_PAGES_PER_SITEMAP + 1;
    const pageNumbers = Array.from({ length: PEOPLE_PAGES_PER_SITEMAP }, (_, i) => start + i).filter(
      (p) => p <= 500
    );
    ...
    const results = await Promise.all(
      pageNumbers.map(async (p) => {
        try {
          const r = await fetch(`${BASE}/person/popular?api_key=${key}&page=${p}`);
          const d = await r.json();
          return (d.results || []).filter(isGoodPerson).map((p2) => `/person/${p2.id}`);
        } catch {
          return [];
        }
      })
